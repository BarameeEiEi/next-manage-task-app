    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const { error: error2 } = await supabase
          .storage
          .from("task_bk")
          .remove([image_url.substring(image_url.lastIndexOf("/") + 1)]); //ลบรูปที่เก็บใน storage โดยใช้ชื่อไฟล์ที่ดึงมาจาก image_url
        ...
        setTasks(tasks.filter((task) => task.id !== id)); //อัพเดท state หลังลบข้อมูล
      }
    })
  }
    ...
    <>
      ...
      >
        ...
        <div className="w-full mt-5 flex justify-end">
          <Link
            href="/addtask"
            className="bg-purple-500 py-2 px-5 rounded
